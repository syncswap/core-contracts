...
import { expandTo18Decimals, getPermitSignature, getSplittedPermitSignature, getWallet, MAX_UINT256, mineBlock, MINIMUM_LIQUIDITY, ZERO, ZERO_ADDRESS } from './shared/utilities';
import { deployDeflatingERC20, v2Fixture } from './shared/fixtures';
...
describe('SyncSwapRouter', () => {
  ...
  it('Should return factory and WETH address', async () => {
    expect(await router.factory()).to.eq(factory.address);
    expect(await router.WETH()).to.eq(WETH.address);
  });

  it('Should add liquidity to volatile pool', async () => {
  ...
  it('Should use expected gas when adding liquidity to volatile pool', async () => {
    const token0Amount = expandTo18Decimals(1)
    const token1Amount = expandTo18Decimals(4)
    ...
    expect(receipt.gasUsed).to.eq(225309)
  })

  it('Should add liquidity with ETH to volatile pool', async () => {
    const WETHPartnerAmount = expandTo18Decimals(1)
    const ETHAmount = expandTo18Decimals(4)

    const expectedLiquidity = expandTo18Decimals(2)
    const WETHPairToken0 = await WETHPair.token0()
    await WETHPartner.approve(router.address, MAX_UINT256)
    await expect(
  ...
  it('Should remove liquidity from volatile pool', async () => {
    const token0Amount = expandTo18Decimals(1)
    const token1Amount = expandTo18Decimals(4)
  ...
  it('Should remove liquidity with ETH from volatile pool', async () => {
    const WETHPartnerAmount = expandTo18Decimals(1)
    const ETHAmount = expandTo18Decimals(4)
    ...
    const expectedLiquidity = expandTo18Decimals(2)
    const WETHPairToken0 = await WETHPair.token0()
    await WETHPair.approve(router.address, MAX_UINT256)
    await expect(
      ...
      .to.emit(WETHPair, 'Transfer')
      .withArgs(WETHPair.address, ZERO_ADDRESS, expectedLiquidity.sub(MINIMUM_LIQUIDITY))
  ...
  it('Should remove liquidity with permit from volatile pool', async () => {
    const token0Amount = expandTo18Decimals(1)
    const token1Amount = expandTo18Decimals(4)
    await addLiquidity(token0Amount, token1Amount)
    ...
    await router.removeLiquidityWithPermit2(
      token0.address,
      token1.address,
      false,
      expectedLiquidity.sub(MINIMUM_LIQUIDITY),
  ...
  it('Should remove liquidity with ETH and permit from volatile pool', async () => {
    const WETHPartnerAmount = expandTo18Decimals(1)
    const ETHAmount = expandTo18Decimals(4)
    await WETHPartner.transfer(WETHPair.address, WETHPartnerAmount)
    await WETH.deposit({ value: ETHAmount })
    ...
    await router.removeLiquidityETHWithPermit2(
      WETHPartner.address,
      expectedLiquidity.sub(MINIMUM_LIQUIDITY),
      0,
      0,
  ...
  describe('swapExactTokensForTokens', () => {
    ...
    it('amounts', async () => {
      await token0.approve(routerEventEmitter.address, MAX_UINT256)
      await expect(
    ...
    it('gas', async () => {
      ...
      const receipt = await tx.wait()
      expect(receipt.gasUsed).to.eq(110372) // 101898
    }).retries(3)
  })
  ...
  describe('swapExactETHForTokens', () => {
    ...
    it('happy path', async () => {
      const WETHPairToken0 = await WETHPair.token0()
      await expect(
        router.swapExactETHForTokens(0, [WETH.address, WETHPartner.address], wallet.address, MAX_UINT256, {
          ...overrides,
          value: swapAmount
        })
      )
    ...
    it('gas', async () => {
      ...
      const receipt = await tx.wait()
      expect(receipt.gasUsed).to.eq(115544) // 138770
    }).retries(3)
  })
  ...
  describe('swapExactTokensForETH', () => {
    ...
    it('amounts', async () => {
      await WETHPartner.approve(routerEventEmitter.address, MAX_UINT256)
      await expect(
  ...
  it('getAmountOut', async () => {
    const swapFee = BigNumber.from(3000);
    expect(await router.getAmountOut(BigNumber.from(2), BigNumber.from(100), BigNumber.from(100), swapFee)).to.eq(BigNumber.from(1))
    await expect(router.getAmountOut(BigNumber.from(0), BigNumber.from(100), BigNumber.from(100), swapFee)).to.be.revertedWith(
      'I' // INSUFFICIENT_INPUT_AMOUNT
    )
    await expect(router.getAmountOut(BigNumber.from(2), BigNumber.from(0), BigNumber.from(100), swapFee)).to.be.revertedWith(
      'L' // INSUFFICIENT_LIQUIDITY
    )
    await expect(router.getAmountOut(BigNumber.from(2), BigNumber.from(100), BigNumber.from(0), swapFee)).to.be.revertedWith(
      'L' // INSUFFICIENT_LIQUIDITY
    )
  })
  ...
  it('getAmountIn', async () => {
    const swapFee = BigNumber.from(3000);
    expect(await router.getAmountIn(BigNumber.from(1), BigNumber.from(100), BigNumber.from(100), swapFee)).to.eq(BigNumber.from(2))
    await expect(router.getAmountIn(BigNumber.from(0), BigNumber.from(100), BigNumber.from(100), swapFee)).to.be.revertedWith(
      'O' // INSUFFICIENT_OUTPUT_AMOUNT
    )
    await expect(router.getAmountIn(BigNumber.from(1), BigNumber.from(0), BigNumber.from(100), swapFee)).to.be.revertedWith(
      'L' // INSUFFICIENT_LIQUIDITY
    )
    await expect(router.getAmountIn(BigNumber.from(1), BigNumber.from(100), BigNumber.from(0), swapFee)).to.be.revertedWith(
      'L' // INSUFFICIENT_LIQUIDITY
    )
  })
...
describe('fee-on-transfer tokens', () => {
  ...
  async function addLiquidity(DTTAmount: BigNumber, WETHAmount: BigNumber) {
    ...
    await router.addLiquidityETH(DTT.address, DTTAmount, DTTAmount, WETHAmount, wallet.address, MAX_UINT256, {
      ...overrides,
      value: WETHAmount
    })
  }
  ...
  it('removeLiquidityETHWithPermitSupportingFeeOnTransferTokens2', async () => {
    const DTTAmount = expandTo18Decimals(1)
      .mul(100)
      .div(99)
    const ETHAmount = expandTo18Decimals(4)
    ...
    await pair.approve(router.address, MAX_UINT256)
    await router.removeLiquidityETHWithPermitSupportingFeeOnTransferTokens2(
      DTT.address,
      liquidity,
      NaiveDTTExpected,
      WETHExpected,
